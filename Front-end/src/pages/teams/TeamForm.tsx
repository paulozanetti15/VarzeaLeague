...
const TeamForm: React.FC = () => {
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-6">{teamId ? 'Editar Time' : 'Criar Time'}</h1>
      ...
      <form onSubmit={handleSubmit} className="max-w-lg">
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-bold mb-2">Nome do Time</label>
          <input
            type="text"
            name="name"
            value={formData.name}
        ...
        <div className="mb-6">
          <label className="block text-gray-700 text-sm font-bold mb-2">Logo do Time</label>
          <input type="file" accept="image/*" onChange={handleImageChange} className="mb-2" />
          {previewImage && <img src={previewImage} alt="Preview" className="w-32 h-32 object-cover rounded" />}
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={() => navigate('/teams')} className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Cancelar</button>
        </div>
      </form>
    </div>
  );
