...
const MatchListing: React.FC = () => {
  ...
  const buildQuery = useMemo(() => {
    ...
    if (filters.myMatches) params.set('myMatches', 'true');
  ...
  const res = await fetch(`${API_BASE_URL}/friendly-matches/search${qs}`);
        let data = [] as any[];
        if (res.ok) {
          data = await res.json();
        } else {
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={3}>
          <MatchFilters
            ...
            onClear={() => setFilters({ searchMatches: '', searchChampionships: '', matchDateFrom: '', registrationDateFrom: '', championshipDateFrom: '', championshipDateTo: '', myMatches: false, statuses: [], sort: 'date_desc', type: getInitialType() })}
