...
const SumulaPartidaAmistosaModal = ({ id, show, onHide, canSave = false }: SumulaPartidaAmistosaModalProps) => {
  ...
  const fetchMatchDetails = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setMatchDate(new Date(match.date).toLocaleDateString('pt-BR'));
        setMatchLocation(match.location || 'Local não informado');
      }
    } catch (error) {
  ...
  const handleExportPDF = () => {
    ...
    doc.text('SÚMULA DA PARTIDA', pageWidth / 2, 20, { align: 'center' });
    
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    doc.text(`Data: ${matchDate}`, 20, 35);
    doc.text(`Local: ${matchLocation}`, 20, 42);
    
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    const placar = `${nomeHomeTeam} ${homeScore} x ${awayScore} ${nomeAwayTeam}`;
    doc.text(placar, pageWidth / 2, 55, { align: 'center' });
    ...
    if (goals.length > 0) {
      ...
      const goalsData = goals.map(g => [g.player, g.team, `${g.minute}'`]);
    ...
    const fileName = `sumula-${nomeHomeTeam.replace(/\s+/g, '-')}-vs-${nomeAwayTeam.replace(/\s+/g, '-')}.pdf`;
    doc.save(fileName);
  };
        ...
        <div onClick={(e) => e.stopPropagation()}>
          {/* Header */}
            ...
            <h2 className="dialog-title">
              {isSaved ? 'Súmula Salva' : 'Súmula da Partida'}
            </h2>
