      ...
      for (const player of playersToUpdate) {
        try {
          ...
          if (!player.id) {
            ...
            // Se existe e não tem vínculo, reutilizar
            if (existingPlayer && (!existingPlayer.teamPlayers || existingPlayer.teamPlayers.length === 0)) {
              // Verificar se já está vinculado ao time atual
